...
export function OTPScreen({ navigation, route }: any) {
  ...
  const { run: verify, loading, error } = useAction(verifyOtp);
  const { run: resend, loading: resending } = useAction(sendOtp);

  const submit = async (value: string) => {
    if (value.length < 6) return;
    // Demo mode: any 6 digits get you in.
    if (!isBackendConfigured) return navigation.replace('ProfileSetup');
    const session = await verify(phone, value);
    // On success AuthProvider flips isAuthed; send new users to pick a handle.
    if (session) navigation.replace('ProfileSetup');
  };
    ...
    <ScreenBackground tone="base" glow={false}>
      ...
      <View style={styles.root}>
        <View style={styles.body}>
          ...
          <Text
            style={styles.resend}
            onPress={() => (isBackendConfigured ? resend(phone) : null)}
          >
